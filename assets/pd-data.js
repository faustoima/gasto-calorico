  ...
  function exportPDF() {
    ...
    readAll().then(function (data) {
      ...
      var html =
        ...
        ". Última avaliação: " + esc(last.date || "—") + "</p>" +

        "<div class='card'>" +
        "Peso: " + esc(last.weight == null ? "—" : last.weight) +
        " kg" + delta(last.weight, first.weight) + "<br>" +
        "Gordura: " + esc(last.bodyFat == null ? "—" : last.bodyFat) +
        "%" + delta(last.bodyFat, first.bodyFat) + "<br>" +
        "Cintura: " + esc(last.waist == null ? "—" : last.waist) +
        " cm" + delta(last.waist, first.waist) + "<br>" +
        "Quadril: " + esc(last.hip == null ? "—" : last.hip) +
        " cm" + delta(last.hip, first.hip) + "<br>" +
        "RMR: " + esc(last.rmrBio == null ? "—" : last.rmrBio) + " kcal" +
        "</div>" +
